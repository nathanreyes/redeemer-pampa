      ...
      <div slot-scope="{}">
        ...
        >
          <div class="border-l-2 border-blue px-6 py-4">
            <h3 class="text-grey-darkest font-semibold mb-3">
              {{ sermon.title }}
            </h3>
            <p class="text-grey-dark text-sm leading-normal">
              Led by
              <span class="text-grey-darker font-semibold">{{
                sermon.leader
              }}</span>
              on
              <span class="text-grey-darker font-semibold">{{
...
export default {
  ...
  methods: {
    async refreshSermons() {
      this.sermons = (
        await import(`@/content/sermons${this.selectedSermonLookup.path}.json`)
